...
export function NewsCommentItem({ comment }: NewsCommentItemProps) {
  return (
    <div className="flex gap-3">
      ...
      <div className="min-w-0 flex-1 rounded-lg border border-dnd-border/60 bg-dnd-dark/40 px-3 py-2.5">
        <div className="flex flex-wrap items-baseline gap-2">
          <span className="text-sm font-medium text-white">{comment.author}</span>
          <span className="text-xs text-dnd-muted">{formatNewsDate(comment.createdAt)}</span>
        </div>
        <RichTextContent content={comment.content} className="mt-1.5" />
      </div>
    </div>
  )
}
